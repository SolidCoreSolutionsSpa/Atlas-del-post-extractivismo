...
**Ubicación**: `src/shared/ui/CircleHotspot.jsx`

**Propósito**: Hotspot circular con borde punteado para mapas de provincias. Marca zonas clickeables de manera visual clara.

**Reutilizable en**: Mapas de provincias, zonas, cualquier mapa con áreas interactivas.

**Props**:
```javascript
<CircleHotspot
  left={string}                    // Posición X en % o px
  top={string}                     // Posición Y en % o px
  label={string}                   // Etiqueta del hotspot
  active={boolean}                 // Default true - si está visible
  onSelect={function}              // Handler al hacer click
  className={string}               // Clases CSS adicionales
/>
```

**Características**:
- Hotspot circular con borde punteado blanco
- Badge interior con etiqueta
- Transición de opacidad suave al cambiar estado active
- Escala en hover
...
**Caso de Uso**:
- ✅ Página de detalle de provincia (hotspots de zonas)
- ✅ Sistema de filtros que afecta visibilidad

---

### 14. **FilterPanel** ⭐

**Ubicación**: `src/casosDeEstudio/ui/FilterPanel.jsx`

**Propósito**: Panel de filtros reutilizable para categorizar elementos (Biótico, Antrópico, Físico). Se posiciona en la esquina inferior izquierda del mapa.

**Reutilizable en**: Cualquier página con elementos categorizados por tipo de afectación.

**Props**:
```javascript
<FilterPanel
  filterDescriptions={Record<string, {title: string, text: string}>}
  activeFilter={string | null}     // Categoría activa
  onFilterChange={function}        // Callback (category) => void
  filterIcons={object}             // Opcional, URLs de iconos por categoría
/>
```

**filterDescriptions Structure**:
```javascript
{
  biotic: {
...
**Características**:
- 3 botones con iconos SVG
- Hover muestra descripción (tooltip)
- Oculta descripción al salir
- Diseño responsivo (flex)
- Fondo semi-transparente (white/90)
...
export function ProvinceDetailPage() {
  ...
  return (
    <InteractiveMap imageSrc={caseStudy.detailMap.image}>
      {/* Decoraciones y hotspots */}

      <FilterPanel
        filterDescriptions={caseStudy.detailMap.filterDescriptions}
...
```javascript
// Los hotspots responden al filtro:
<CircleHotspot
  active={!activeFilter || activeFilter === spot.category}
  // ...
/>

// Las decoraciones responden al filtro:
<MapDecoration
  className={clsx(
    activeFilter && activeFilter !== decoration.category
      ? 'opacity-30'
      : 'opacity-95'
  )}
  // ...
